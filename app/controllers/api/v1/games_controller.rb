class Api::V1::GamesController < ApplicationController
  ...
  def update
    if (!!params["wight"])
      @game.update(wight:params["wight"])
    else
      all_characters={
        shadow:[
          {
      ...
      # fieldCard ={2:"Hermit's Cabin", 3:"Hermit's Cabin", 4:"Underworld Gate", 5:"Underworld Gate", 6:"Church", 8:"Cemetery", 9:"Werid woods", 10:"Erstwhile Altar"}

      @game.update(turn: params["turn"]["id"])
      @player = Player.find(params["turn"]["id"])
      if @player.character.name === "none"
        current_field = [[2,3],[4,5],6, 8, 9, 10].shuffle.flatten
        @game.update(field:current_field)
      ...
      @player = Player.find(params["turn"]["id"])
      if @game.save
        if @game.wight === nil || @game.wight === 0
          ...
          @game.update(wight:@game.wight-1)
        end
  ...
  def game_params
    params.require(:game).permit(:id,:turn, :wight, :field)
  end
